  ...
  <b-overlay :show="confirmDelete" rounded="sm">
    <b-overlay :show="saving" rounded="sm" spinner-small class="mt-2">
      <b-input-group class="mb-2">
        ...
        <b-form-input
          v-else
          type="text"
          :value="$helpers.formatPrice(gym.settings, localPrice.salesPrice)"
          :disabled="!editing"
          :readonly="!editing"
        />
        <b-form-select
          v-model="localPrice.vatRegime"
          :options="vatFormOptions"
          :disabled="!editing"
        ></b-form-select>
    ...
    <template v-slot:overlay>
      <save-button
        ...
        variant="danger"
        :saving="deleting"
        :saveText="$t('form.deleting')"
        @click="deletePrice()"
      >{{ $t('form.delete') }}</save-button>
      <b-btn
        size="sm"
        variant="secondary"
