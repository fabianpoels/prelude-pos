<template>
  <div>
    <b-dropdown variant="link-secondary" no-caret>
      ...
      <b-dropdown-item v-if="canDeleteCategory(category)" v-b-modal="`deleteCategory-${category._id}`">{{ $t('form.delete') }}</b-dropdown-item>
      <b-dropdown-item v-else :disabled="!canArchiveCategory(category)" v-b-modal="`deleteCategory-${category._id}`">{{ $t('form.archive') }}</b-dropdown-item>
    </b-dropdown>
